...
export class ReportesRepository {
  ...
  async aggregateResumenGlobal(filtros: ReporteSqlFiltros): Promise<RawResumenFinancieroRow> {
    ...
    const rows = await this.db.$queryRaw<RawResumenFinancieroRow[]>`
      SELECT
        CAST(COALESCE(SUM(vf.valor_aplicado), 0) AS CHAR) AS total_generado,
        CAST(COALESCE(SUM(CASE WHEN vf.facturado = 0 THEN vf.valor_aplicado ELSE 0 END), 0) AS CHAR) AS pendiente_facturacion,
        CAST(COALESCE(SUM(CASE WHEN vf.facturado = 1 AND vf.pagado = 0 THEN vf.valor_aplicado ELSE 0 END), 0) AS CHAR) AS facturado_pendiente_pago,
        CAST(COALESCE(SUM(CASE WHEN vf.pagado = 1 THEN vf.valor_aplicado ELSE 0 END), 0) AS CHAR) AS pagado,
