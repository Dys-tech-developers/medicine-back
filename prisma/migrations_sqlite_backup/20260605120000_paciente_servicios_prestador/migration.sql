-- Paciente servicios: prestador responsable por asignación
...
SELECT
    ...
    COALESCE(
        (
            SELECT "v"."prestador_id"
            FROM "visitas" AS "v"
            WHERE "v"."paciente_servicio_id" = "paciente_servicios"."id"
            ORDER BY "v"."id" ASC
            LIMIT 1
        ),
        (
            SELECT "ps"."prestador_id"
            FROM "prestador_servicios" AS "ps"
            WHERE "ps"."servicio_id" = "paciente_servicios"."servicio_id"
            ORDER BY "ps"."prestador_id" ASC
            LIMIT 1
        )
    ),
    "fecha_inicio",
